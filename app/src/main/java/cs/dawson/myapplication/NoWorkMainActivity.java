...
public class NoWorkMainActivity extends AppCompatActivity {
    ...
    private void loadArrays() {
        ...
        questions = generateInvalidArray(4);
    ...
    private int[] generateInvalidArray(int size){

        Random rand = new Random();
        usedImgs = new ArrayList();

        while (usedImgs.size()!=4){
            int r = rand.nextInt(size);
            if (!usedImgs.contains(r)){
                usedImgs.add(r);
            }
        }

        int[] used = new int[usedImgs.size()];

        for(int i  = 0; i<usedImgs.size(); i++){
            used[i] = (int) usedImgs.get(i);
        }
        return used;
    }
    ...
    private int setGrid() {
        ...
        grid = new int[4];
        int[] used = generateInvalidArray(10);
        int correct = questions[setgridquestions];


        Random rand = new Random();
        int randCorrect = rand.nextInt(4);

        int questiongrid = questions[setgridquestions];

        grid[randCorrect] = questionIMG[questiongrid];
...
//        return randyCorrect;
    ...
    private void drawGrid(int correct){
        ...
        qOutOf.setText(outOf);// was here
                buttons[correct].setBackgroundResource(grid[questiongrid]);
